...
# Step 1: Create an isolated HOME so the installer runs against a fresh
# user-space layout and not any existing developer install.
RUN_ROOT="${RUNNER_TEMP:-$(mktemp -d)}"
export HOME="$RUN_ROOT/msa-macos-home"
mkdir -p "$HOME"

# Step 2: Stage the real-media validation payload outside the installed app.
# These tests are collateral for validation, not part of the bundle payload.
...
# Step 3: Run the real bundle installer exactly as an end user would.
...
APP_DIR="$HOME/Applications/MediaSearchAgent.app/Contents/Resources"
DATA_DIR="$HOME/Library/Application Support/MediaSearchAgent"
CACHE_DIR="$HOME/Library/Caches/MediaSearchAgent"
CONFIG_PATH="$HOME/Library/Application Support/MediaSearchAgent/config.yaml"
...
# config dir matches the install location on macOS, so HOME override
# (Step 1) is sufficient — no MSA_CONFIG_PATH needed. MSA_DEVICE=cpu
# avoids MPS-OOM on the macos-14 runner: device "auto" resolves to MPS,
# and CLIP / RT-DETR / facenet loaded back-to-back blow the runner's
# tight MPS budget.
...
# (lazy-load on first /search), and on slower runners (notably macos-14)
# cold-start encoding exceeds pytest's 20s urlopen timeout in
# test_search_endpoint_returns_json. A throwaway POST /search with a
# generous timeout absorbs the cold start outside the test budget;
# subsequent /search calls hit the warm model.
